...
//});
//
$app->post('/resolve',function(Request $request, Response $response){
    $ResponseData=new stdClass;
    $data=$request->getParsedBody();
    $user_redis = new Redis();
    $user_redis->connect('master_company_cache', 6379);
    $ResponseData->company=$user_redis->get($data['username']);
    ...
    if(!$ResponseData->company)
        {
            ...
            $stmt = $pdo->prepare("select company from users where username=?");
