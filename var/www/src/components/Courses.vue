<template>
  <CContainer>
    ...
    <font-awesome-icon icon="fas fa-database"/>
      <!--{{this.$data.run.database_obj.courses_id}}-->
      COURSES
      <CButton color="success">new</CButton>{{" "}}
    <CButton color="warning">manage</CButton>{{" "}}
    <CButton color="secondary" v-on:click="this.getCourses(this.$store.etc.token)">refresh</CButton>
    </h3>
    <!--<a>(id={{this.$data.run.course_list}})</a>-->
    </CRow>
    <CRow>
      <vue-good-table
...
export default {
  ...
  created() {
    ...
    this.getCourses(this.$store.etc.token);
  },
  methods : {
  onRowClick(params){
 ...
 //     });
  ...
  getCourses(token)
    ...
    $.ajax({
      type: 'POST',
      data: send, 
      url: this.$store.etc.rest+"/etc/courses",
      success:
      (response) =>
          {
            ...
            var a=JSON.parse(atob(response)).courses;
            var t = JSON.parse(JSON.stringify(a));
            this.$data.run.table_columns.push({label:"Course ID",field:"course_id",type:"text"});
            this.$data.run.table_columns.push({label:"Course Name",field:"course_name",type:"text"});
            this.$data.run.table_columns.push({label:"Owner",field:"owner",type:"text"});
            a.forEach((course)=>{
              console.log(course);
              this.$data.run.table_rows.push(course);
              //var t={};
              //t.course_id=
            });
           // this.$data.run.course_data=a;
           // this.$data.run.course_obj.db_fields.forEach((field)=>{
